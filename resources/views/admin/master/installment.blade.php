                                ...
                                <div class="card-header">
                                    <span class="float-right">
                                        <button class="btn btn-info float-right m-1" data-toggle="modal"
                                            data-target="#modal-installment">Add Installment</button>
            ...
            <div class="modal fade" id="modal-import">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="" id="InstallmentImportForm" method="POST" enctype="multipart/form-data">
                            @csrf
...
@section('custom_js')
    ...
    <script>
        $(function() {
            let table = $('#installmentTable').DataTable({
                processing: true,
                serverSide: true,

                ajax: {
                    url: "{{ route('installment.list') }}",
                    data: function(d) {
                ...
                buttons: [{
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        let url = "{{ route('installment.export') }}" + "?search=" +
        ...
        $('#modal-installment').on('show.bs.modal', function(event) {
            let rowData = $(event.relatedTarget).data('row');

            if (rowData) {
                $('#installment_id').val(rowData.id);
        ...
        $('#importBtn').on('click', function() {
            let formData = new FormData($('#InstallmentImportForm')[0]);
            $.ajax({
                url: "{{ route('import.installment') }}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            toastr.success(response.message);
                            $('#installmentTable').DataTable().ajax.reload();
                        }
                    });
